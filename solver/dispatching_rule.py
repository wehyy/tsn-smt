...
class PriorityDispatchSolver(JSSolver):
    ...
    def solving_iteration(self, solution:CFSSolution):
        ...
        while head_ops:
            ...
            next_flow_op = op.next_flow_op
            if next_flow_op is None:
                head_ops = head_ops[0:pos] + head_ops[pos+1:]
            else:
                head_ops[pos] = next_flow_op
...
class DisPatchingRules:
    ...
    # - Total Processing Time : Total time required to complete a job. 
    @staticmethod
    def PT(op:TransOperationStep, solution:CFSSolution):
        '''Processing Time.'''
        return op.source.duration

    @staticmethod
    def PS(op:TransOperationStep, solution:CFSSolution):
        '''Process Sequence.'''
        flow = solution.flow_head(op)
        return len(solution.flow_ops[flow])   

    @staticmethod
    def TPT(op:TransOperationStep, solution:CFSSolution):
        '''Total Processing Time.'''
        flow = solution.flow_head(op)
        return sum(op.source.duration for op in solution.flow_ops[flow]) 
    ...
    # - Total Work Remaining  : The time for a job to complete the remaining operations. 
    @staticmethod
    def CT(op:TransOperationStep, solution:CFSSolution):
        '''Creation Time.'''
        return op.pre_flow_op.end_time

    @staticmethod
    def WT(op:TransOperationStep, solution:CFSSolution):
        '''Waiting Time.'''
        arrive_time = op.pre_flow_op.end_time
        machine_time = solution.edge_head(op).tailed_edge_op.end_time
        return max(machine_time-arrive_time, 0)
    
    @staticmethod
    def TWR(op:TransOperationStep, solution:CFSSolution):
        '''Total Work Remaining.'''
        num = 0
        ref = op
        while ref:
            num += ref.source.duration
            ref = ref.next_flow_op
        return num
    ...
    # ------------------------------
    @staticmethod
    def SPT(op:TransOperationStep, solution:CFSSolution):
        '''Shortest Processing Time.'''
        return DisPatchingRules.PT(op, solution)

    @staticmethod
    def LPT(op:TransOperationStep, solution:CFSSolution):
        '''Longest Processing Time.'''
        return -DisPatchingRules.PT(op, solution)
    
    @staticmethod
    def SPS(op:TransOperationStep, solution:CFSSolution):
        '''Shortest Process Sequence.'''
        return DisPatchingRules.PS(op, solution)

    @staticmethod
    def LPS(op:TransOperationStep, solution:CFSSolution):
        '''Longest Process Sequence.'''
        return -DisPatchingRules.PS(op, solution)
    
    @staticmethod
    def STPT(op:TransOperationStep, solution:CFSSolution):
        '''Shortest Total Processing Time.'''
        return DisPatchingRules.TPT(op, solution)
    
    @staticmethod
    def LTPT(op:TransOperationStep, solution:CFSSolution):
    ...
    # ------------------------------
    @staticmethod
    def ECT(op:TransOperationStep, solution:CFSSolution):
        '''Earliest Creation Time.'''
        return DisPatchingRules.CT(op, solution)
    
    @staticmethod
    def LCT(op:TransOperationStep, solution:CFSSolution):
        '''Longest Creation Time.'''
        return -DisPatchingRules.CT(op, solution)
    

    @staticmethod
    def SWT(op:TransOperationStep, solution:CFSSolution):
        '''Shortest Waiting Time.'''
        return DisPatchingRules.WT(op, solution)
    
    @staticmethod
    def LWT(op:TransOperationStep, solution:CFSSolution):
        '''Longest waiting Time.'''
        return -DisPatchingRules.WT(op, solution)

    @staticmethod
    def LTWR(op:TransOperationStep, solution:CFSSolution):
        '''Least Total Work Remaining.'''
        return DisPatchingRules.TWR(op, solution)

    @staticmethod
    def MTWR(op:TransOperationStep, solution:CFSSolution):
        '''Most Total Work Remaining.'''
        return -DisPatchingRules.TWR(op, solution)


    @staticmethod
    def EST(op:TransOperationStep, solution:CFSSolution):
        '''Earliest Start Time.'''
        return solution.estimated_start_time(op)
    
    @staticmethod
    def LST(op:TransOperationStep, solution:CFSSolution):
        '''Longest Start Time.'''
        return -DisPatchingRules.EST(op, solution)


    @staticmethod
    def HH(op:TransOperationStep, solution:CFSSolution):
    ...
    @staticmethod
    def IHH(op:TransOperationStep, solution:CFSSolution):
        '''Improved HH rule.'''
