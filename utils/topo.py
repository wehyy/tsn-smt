...
class Topo():
    ...
    def initRandomTopo(self, nodes_num:int=30):
        self._nodes = [Node(i) for i in range(1, nodes_num + 1)]
        _edges, self._into_degree, self._out_degree, self._position = self.__generate__DAG(nodes_num=nodes_num, max_out=4, alpha=1.2, beta=2.0)
        self._graph.add_edges_from(_edges)
        edges = []
        for id, _edge in enumerate(_edges):
            edge = Edge(id, Node(_edge[0]), Node(_edge[1]))
            edges.append(edge)
        self._edges = edges
            
        
    def initRandomWeightedTopo(self, nodes_num:int=30):
        self._nodes = [Node(i) for i in range(1, nodes_num + 1)]
        _edges, self._into_degree, self._out_degree, self._position = self.__generate__DAG(nodes_num=nodes_num, max_out=4, alpha=1.2, beta=2.0)
